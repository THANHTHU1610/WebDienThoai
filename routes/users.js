...
router.get("/", (req, res) => {
  ...
  if (query) {
    const users = getUsersByName(query);
    res.json(users);
  } else {
    res.json({ message: "Please provide a query parameter" });
  }
});

function getUsersByName(name) {
  const filteredUsers = users.filter((user) => user.username.includes(name));
